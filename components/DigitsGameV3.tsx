...
const SYMBOLS: Record<string,string> = {
  R_100:'Volatility 100 Index', R_50:'Volatility 50 Index', R_10:'Volatility 10 Index',
  R_25:'Volatility 25 Index', R_75:'Volatility 75 Index', '1HZ100V':'Volatility 100 (1s) Index', '1HZ50V':'Volatility 50 (1s) Index'
};
...
function playSound(kind:'tick'|'buy'|'win'|'loss') {
  if (typeof window === 'undefined') return;
  try {
    unlockAudio();
    if (!audioCtx || !audioUnlocked) return;
    const notes = kind==='win' ? [660,880,1046] : kind==='loss' ? [440,330,220] : kind==='buy' ? [520,760] : [620];
    ...
    notes.forEach((freq,i)=>{
      ...
      gain.gain.exponentialRampToValueAtTime(kind==='tick' ? 0.035 : 0.075, now + i*0.08 + 0.012);
      gain.gain.exponentialRampToValueAtTime(0.0001, now + i*0.08 + 0.12);
      osc.connect(gain); gain.connect(audioCtx!.destination);
      osc.start(now + i*0.08); osc.stop(now + i*0.08 + 0.13);
    });
  } catch {}
}
...
export default function DigitsGameV3() {
  ...
  useEffect(()=>{
    ...
    localStorage.setItem('mozhyper-theme',theme);
  },[theme]);
  ...
  useEffect(()=>{
    ...
    return()=>{window.removeEventListener('pointerdown',activate);window.removeEventListener('touchstart',activate);window.removeEventListener('keydown',activate)};
  },[]);

  useEffect(()=>{ if(isConnected) subscribeTicks(symbol); },[symbol,isConnected,subscribeTicks]);
  useEffect(()=>{
    if(isAuthorized && isConnected && !tradeActive) getProposal(symbol,CONTRACT_TYPES[contractType],Math.max(MIN_STAKE,amount),5,digit);
  },[contractType,amount,digit,symbol,isAuthorized,isConnected,tradeActive,getProposal]);

  useEffect(()=>{
    if(!tradeActive || !tick?.epoch || lastTick.current===tick.epoch) return;
    lastTick.current=tick.epoch;
    playSound('tick');
  ...
  useEffect(()=>{
    ...
    const id=tx?.contract_id ?? null;
    if(id && id!==lastClosed.current){
      lastClosed.current=id;
      setTradeActive(false);
      setCountdown(5);
      const pnl=Number(tx.profit_loss ?? 0);
      setPopup({id,pnl});
      playSound(pnl>=0?'win':'loss');
      window.setTimeout(()=>setPopup(p=>p?.id===id ? null : p),3500);
    }
  },[profitTransactions]);

  useEffect(()=>{
    if(error && tradeActive && !buying){ setTradeActive(false); setCountdown(5); }
  },[error,tradeActive,buying]);
  ...
  const last = profitTransactions[0];
  const lastPnl = Number(last?.profit_loss ?? 0);
  ...
  const labels:Record<ContractChoice,string> = language==='pt'
    ? {MATCH:'Igual',DIFFERS:'Diferente',OVER:'Acima',UNDER:'Abaixo'}
    : language==='es' ? {MATCH:'Igual',DIFFERS:'Diferente',OVER:'Superior',UNDER:'Inferior'}
    : {MATCH:'Matches',DIFFERS:'Differs',OVER:'Over',UNDER:'Under'};

  const place=()=>{
    if(!proposal || !isAuthorized || buying || tradeActive || Number(proposal.ask_price)<MIN_STAKE) return;
    unlockAudio();
    playSound('buy');
    setTradeActive(true); setCountdown(5); lastTick.current=tick?.epoch ?? null;
    buy(proposal.id,proposal.ask_price);
  };
  const stake=(v:string)=>{
    setAmountText(v);
    const n=Number(v.replace(',','.'));
    if(Number.isFinite(n) && n>=MIN_STAKE) setAmount(n);
  };
  const normalize=()=>{
    const n=Number(amountText.replace(',','.'));
    const safe=Number.isFinite(n) ? Math.max(MIN_STAKE,n) : MIN_STAKE;
    setAmount(safe); setAmountText(safe.toFixed(2));
  };
  ...
  const cashier=()=>window.open('https://app.deriv.com/cashier','_blank','noopener,noreferrer');
  ...
  return <div className="matos-screen">
    ...
    <button className="cta" onClick={place} disabled={!proposal||!isAuthorized||buying||tradeActive}>{buying?`⏳ ${t('buying')}`:language==='pt'?'🎯 Colocar previsão':language==='es'?'🎯 Colocar predicción':'🎯 Place prediction'}{proposal&&<small> · +{Number(proposal.payout-proposal.ask_price).toFixed(2)} USD</small>}</button>
