...
type Signal = { contract: Contract; pct100: number; pct7: number; reason: string };
...
function pct(ticks: number[], predicate: (d: number) => boolean) { const a = ticks.map(digit).filter((x): x is number => x !== null); return a.length ? a.filter(predicate).length / a.length * 100 : 0; }
function buildSignal(ticks: number[]): Signal | null {
  if (ticks.length < 7) return null;
  const a = ticks.slice(-100), m = ticks.slice(-7);
  const even100 = pct(a, d => d % 2 === 0), even7 = pct(m, d => d % 2 === 0);
  const over100 = pct(a, d => d > 4), over7 = pct(m, d => d > 4);
  if (even7 >= 60 && even100 > 49) return { contract: 'EVEN', pct100: Math.round(even100), pct7: Math.round(even7), reason: `PAR · 100T ${Math.round(even100)}% > 49% · 7T ${Math.round(even7)}%` };
  if (100 - even7 >= 60 && 100 - even100 > 49) return { contract: 'ODD', pct100: Math.round(100 - even100), pct7: Math.round(100 - even7), reason: `ÍMPAR · 100T ${Math.round(100 - even100)}% > 49% · 7T ${Math.round(100 - even7)}%` };
  if (over7 >= 60 && over100 > 49) return { contract: 'OVER', pct100: Math.round(over100), pct7: Math.round(over7), reason: `ACIMA 4 · 100T ${Math.round(over100)}% > 49% · 7T ${Math.round(over7)}%` };
  if (100 - over7 >= 60 && 100 - over100 > 49) return { contract: 'UNDER', pct100: Math.round(100 - over100), pct7: Math.round(100 - over7), reason: `ABAIXO 5 · 100T ${Math.round(100 - over100)}% > 49% · 7T ${Math.round(100 - over7)}%` };
  return null;
}
...
function DigitCandles({ ticks }: { ticks: number[] }) {
  ...
  return <div className="digit-candle-panel">
    <div className="digit-candle-title"><span>Dígitos · últimos 100 ticks</span><small>MT5</small></div>
    <div className="digit-candles">
      {dist.map((p, i) => {
        const high = p >= 50;
        const height = Math.max(4, Math.min(48, Math.round(Math.abs(p - 50) * .95)));
...
function BotWorker({ symbol, accountType, lossLimitMzn, targetMzn, baseStake, setSharedActivity }: { symbol: string; accountType: 'demo' | 'real'; lossLimitMzn: number; targetMzn: number; baseStake: number; setSharedActivity: (s: string) => void }) {
  ...
  return <div className="bot-worker">
    ...
    <div className="auto-status-card">
      <div className="luxury-bot-icon"><span>◈</span></div>
      ...
      <div className="auto-sub">100 ticks: filtro de contexto · 7 ticks: gatilho · {isConnected ? 'Ligado à Deriv' : 'A ligar'} · {accountType === 'real' ? 'Conta Real' : 'Conta Demo'}</div>
    </div>
    <div className="auto-metrics"><div><b>{ticks.length}/100</b><span>Ticks</span></div><div><b>{signal ? `${signal.pct7}%` : '—'}</b><span>Força 7T</span></div><div><b className={sessionProfit >= 0 ? 'up' : 'down'}>{sessionProfit >= 0 ? '+' : ''}{(sessionProfit * USD_TO_MZN).toFixed(0)} MT</b><span>Resultado</span></div><div><b>{losses} L</b><span>Perdas</span></div></div>
    <div className="auto-section-title">Estratégia <small>{signal ? signal.reason : 'Aguardando concordância'}</small></div>
    ...
    <div className="stake-read"><span>Stake actual</span><b>${stake.toFixed(2)}</b></div>
...
export default function AutoBot() {
  ...
  `}</style>
    ...
    <div className="stake-control"><span style={{ fontSize: 9, color: 'var(--t3)' }}>Stake inicial</span><button onClick={() => changeStake(-.1)}>−</button><b>${stake.toFixed(2)}</b><button onClick={() => changeStake(.1)}>+</button></div>
