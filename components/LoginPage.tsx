...
export default function LoginPage() {
  ...
  const copy = {
    en: {
      subtitle: 'Sign in with your Deriv account to start trading.',
      login: 'Login with Deriv',
      noAccount: "Don't have a Deriv account?",
      signup: 'Create Deriv account →',
      note: 'Your credentials are securely handled via OAuth 2.0.',
      risk: 'Risk warning:',
    ...
    pt: {
      subtitle: 'Entre com a sua conta Deriv para começar a negociar.',
      login: 'Entrar com Deriv',
      noAccount: 'Não tem uma conta Deriv?',
      signup: 'Criar conta Deriv →',
      note: 'As suas credenciais são tratadas com segurança através do OAuth 2.0.',
      risk: 'Aviso de risco:',
    ...
    es: {
      subtitle: 'Inicia sesión con tu cuenta Deriv para comenzar a operar.',
      login: 'Iniciar sesión con Deriv',
      noAccount: '¿No tienes una cuenta Deriv?',
      signup: 'Crear cuenta Deriv →',
      note: 'Tus credenciales se gestionan de forma segura mediante OAuth 2.0.',
  ...
  return (
    <main className="moz-login-page">
      ...
      <div className="moz-login-card">
        <div className="moz-lang-switch" aria-label="Language">
        ...
        <div className="moz-login-brand">
          <div className="moz-login-mark">M</div>
          <div>
            <div className="moz-login-name">Moz<span>Hyper</span></div>
            <div className="moz-login-sub">DIGITS TRADING</div>
          </div>
        </div>

        <p className="moz-login-intro">
          {t('loginSubtitle') || copy.subtitle}
        </p>

        <button
          type="button"
          className="moz-login-btn"
          onClick={() => { window.location.assign('/api/auth/login'); }}
        >
          🔒 {t('loginWithDeriv') || copy.login}
        </button>

        <div className="moz-login-divider" />

        <div className="moz-signup-row">{copy.noAccount}</div>
        <a
          href={DERIV_SIGNUP_URL}
          className="moz-signup-link"
          target="_blank"
        ...
        <div className="moz-oauth-note">{copy.note}</div>
      </div>

      <div className="moz-risk-card">
        <p>
          <strong>{copy.risk}</strong> {copy.riskText}
        </p>
      </div>
    </main>
  );
