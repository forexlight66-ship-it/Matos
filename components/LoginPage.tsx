...
const redDeriv = (text: string) => {
  ...
  return <>{parts.map((p, i) => p === 'Deriv' ? <span key={i} style={{ color: '#ff4d5f' }}>{p}</span> : p)}</>;
};

const css: Record<string, React.CSSProperties> = {
  page: { minHeight: '100vh', width: '100%', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '28px 16px', background: '#070b16', color: '#f5f7fb', fontFamily: 'Inter, system-ui, sans-serif' },
  ...
  blue: { color: '#3d7fff' },
  sub: { marginTop: 7, color: '#74809b', fontSize: 9, fontWeight: 800, letterSpacing: '.20em' },
  ...
  divider: { height: 1, background: 'rgba(255,255,255,.07)', margin: '23px 0 18px' },
  signup: { textAlign: 'center', color: '#8d99b2', fontSize: 12 },
  signupLink: { display: 'block', textAlign: 'center', marginTop: 8, color: '#5e98ff', fontSize: 13, fontWeight: 800, textDecoration: 'none' },
  ...
  risk: { position: 'relative', zIndex: 2, width: 'min(100%, 430px)', marginTop: 12, padding: '13px 15px', borderRadius: 15, background: 'rgba(240,73,90,.07)', border: '1px solid rgba(240,73,90,.18)', color: '#9b7180', fontSize: 9.5, lineHeight: 1.55 },
};
...
export default function LoginPage() {
  ...
  return (
    <main style={css.page}>
      <div style={css.glowBlue} />
      <div style={css.glowGold} />
      <div style={css.grid} />
      <section style={css.card}>
        <div style={css.lang} aria-label="Language">
          {(['en', 'pt', 'es'] as const).map(item => (
            <button key={item} type="button" onClick={() => setLanguage(item)} style={{ border: 0, borderRadius: 7, padding: '5px 7px', cursor: 'pointer', color: lang === item ? '#fff' : '#75819b', background: lang === item ? '#315fc9' : 'transparent', fontSize: 9, fontWeight: 800 }}>{item.toUpperCase()}</button>
          ))}
        </div>
        ...
        <div style={css.brand}>
          ...
          <div>
            <div style={css.brandName}>Moz<span style={css.blue}>Hyper</span></div>
