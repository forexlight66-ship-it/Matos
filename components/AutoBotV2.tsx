...
const STRATEGIES: { id: Strategy; label: string; icon: string }[] = [
  ...
  { id: 'ACIMA_RISE', label: 'Acima + Subir', icon: '↗' },
  ...
  return (
    <div className="robot-page">
      <style>{`
        ...
        .row3{display:grid;grid-template-columns:1fr 1fr 52px;gap:8px}.chip{min-width:0;border:1px solid var(--rline);border-radius:12px;padding:10px 11px;background:var(--rbg1)}.chip.demo{background:linear-gradient(135deg,#173029,#10241f);border-color:rgba(40,224,160,.25)}.chip.demo .select{color:var(--rem)}.pause{display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#3a2020,#2a1616);border-color:rgba(255,107,94,.3);font-size:18px;color:var(--rcoral);cursor:pointer}.pause.running{background:linear-gradient(135deg,#183b31,#123027);border-color:rgba(40,224,160,.3);color:var(--rem)}
        .select-card{display:flex;flex-direction:column;justify-content:center;gap:4px}.select{width:100%;background:transparent;border:0;outline:0;color:var(--rhi);font-weight:800;font-size:12px;appearance:auto}.select option{background:#111823;color:#fff}.select-card:focus-within{border-color:var(--rblue);box-shadow:0 0 0 2px rgba(63,140,240,.12)}
        ...
        .duration{display:flex;gap:8px;margin-top:7px}.duration button{flex:1;padding:12px 4px;border-radius:11px;border:1px solid var(--rline);background:var(--rbg1);color:var(--rmid);font-family:ui-monospace,SFMono-Regular,monospace;font-size:12px;font-weight:800;cursor:pointer}.duration button.on{background:linear-gradient(135deg,#3a6fc9,var(--rblue));color:#fff;border-color:#5a9af5}.duration button:disabled{opacity:.55;cursor:not-allowed}
        .history{padding:13px 14px}.history-list{margin-top:8px;display:flex;flex-direction:column;gap:6px;max-height:180px;overflow:auto}.hist{display:grid;grid-template-columns:52px 1fr auto;gap:7px;align-items:center;border-bottom:1px solid var(--rline);padding:7px 0;font-size:9px}.hist:last-child{border-bottom:0}.hist-time,.hist-type{color:var(--rmid)}.hist-result.pos{color:var(--rem)}.hist-result.neg{color:var(--rcoral)}.empty{padding:18px;text-align:center;color:var(--rlow);font-size:11px}.footer{text-align:center;padding:15px 0 0;color:var(--rlow);font-family:ui-monospace,SFMono-Regular,monospace;font-size:9px}
        @media(max-width:360px){.row3{grid-template-columns:1fr 1fr 46px;gap:6px}.chip{padding:9px 8px}.select{font-size:11px}.wheel{width:calc(100vw - 30px)}.bubble{width:50px;height:50px}}
      `}</style>
      <div className="content">
        ...
        <div className="row3">
          <label className="chip select-card demo"><div className="lbl">Tipo de Conta</div><select className="select" value={accountType} onChange={e=>{setAccountType(e.target.value as 'demo'|'real');resetAnalysis();}} disabled={running}><option value="demo">Demo</option><option value="real">Real</option></select></label>
          <label className="chip select-card"><div className="lbl">Estratégia</div><select className="select" value={strategy} onChange={e=>{setStrategy(e.target.value as Strategy);resetAnalysis();}} disabled={running}>{STRATEGIES.map(s=><option key={s.id} value={s.id}>{s.label}</option>)}</select></label>
          <button className="chip pause" onClick={running ? stop : start} aria-label={running ? 'Parar robô' : 'Iniciar robô'}>{running ? '■' : '▶'}</button>
        </div>
        ...
        <label className="card select-card" style={{padding:'13px 14px'}}><div className="lbl">Símbolo</div><select className="select" value={selectedSymbol} onChange={e=>{setSelectedSymbol(e.target.value);resetAnalysis();}} disabled={running}>{SYMBOL_OPTIONS.map(s=><option key={s} value={s}>{SYMBOLS[s]}</option>)}</select></label>
        ...
        {error&&<div className="card signal"><div className="lbl">Erro</div><div style={{marginTop:6,fontSize:11,color:'var(--rcoral)',lineHeight:1.4}}>{error}</div></div>}
        <div className="footer">Powered by <span style={{color:'var(--rgold)'}}>MozHyper</span> · Negocie com responsabilidade</div>
      </div>
    </div>
  );
}
