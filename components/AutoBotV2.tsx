...
const CONTRACT_TYPES: Record<Signal['contract'], string> = {
...
function makeSignal(values: number[], strategy: Strategy): Signal | null {
  ...
  if (strategy === 'PAR_ACIMA') {
    if (parity && overUnder && parity.strength >= threshold && overUnder.strength >= threshold) return { ...parity, label: `${parity.label} + ${overUnder.label}`, strength: Math.round((parity.strength + overUnder.strength) / 2) };
    return null;
  }
  if (strategy === 'PAR_RISE') {
    if (parity && direction) return { ...parity, label: `${parity.label} + ${direction.label}`, strength: Math.round((parity.strength + direction.strength) / 2) };
    return null;
  }
  if (strategy === 'ACIMA_RISE') {
    if (overUnder && direction) return { ...overUnder, label: `${overUnder.label} + ${direction.label}`, strength: Math.round((overUnder.strength + direction.strength) / 2) };
    return null;
  }
  if (parity && overUnder && direction) {
    return { ...parity, label: `${parity.label} + ${overUnder.label} + ${direction.label}`, strength: Math.round((parity.strength + overUnder.strength + direction.strength) / 3) };
  }
  return null;
}
...
export default function AutoBotV2() {
  ...
  useEffect(() => {
    ...
    setTicks(prev => {
      ...
      setCycle(count >= windowSize ? 0 : count);
      if (count >= windowSize) {
        const s = makeSignal(next, strategy);
        setSignal(s);
        setLog(v => [`${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })} · ${s ? `SINAL ${s.label} ${s.strength}%` : 'SEM SINAL'} · ${windowSize}T`, ...v].slice(0, 10));
        return [];
      }
      return next;
    });
  ...
  useEffect(() => {
    ...
    const ok = getProposal(symbol, type, Math.max(0.5, stake), safeDuration, barrier);
  ...
  const start = () => {
    ...
    setTicks([]); setCycle(0); setSignal(null); setLog([]); requested.current = false; setRunning(true);
  };
  const stop = () => { setRunning(false); requested.current = false; };
  ...
  return <div className="bot-worker" style={{ padding: 12 }}>
    ...
    <div className="v2-card">
      <div className="v2-row">
        <div><div className="v2-label">Mercado</div><select className="v2-select" value={symbol} onChange={e => { setSymbol(e.target.value); setTicks([]); setCycle(0); }} disabled={running}>{SYMBOL_OPTIONS.map(s => <option key={s} value={s}>{SYMBOLS[s]}</option>)}</select></div>
        <div><div className="v2-label">Conta</div><select className="v2-select" value={accountType} onChange={e => setAccountType(e.target.value as 'demo' | 'real')} disabled={running}><option value="demo">Demo</option><option value="real">Real</option></select></div>
      </div>
    </div>

    <div className="v2-card">
      <div className="v2-label">Análise por ticks</div>
      <div className="v2-row">{([5,10,25,100] as WindowSize[]).map(n => <button key={n} onClick={() => { setWindowSize(n); setTicks([]); setCycle(0); setSignal(null); }} style={{ padding:'9px 5px', borderRadius:9, border:'1px solid rgba(255,255,255,.08)', background:windowSize===n?'var(--blue)':'var(--s1)', color:windowSize===n?'#fff':'var(--t1)', fontWeight:900, cursor:'pointer' }}>{n}T</button>)}</div>
      <div style={{ marginTop:10 }}><div className="v2-bar"><div className="v2-fill" style={{ width:`${Math.min(100, cycle / windowSize * 100)}%` }} /></div><div className="v2-cycle">{cycle}/{windowSize}</div></div>
      <small style={{ color:'var(--t3)' }}>Ao chegar ao limite, o contador reinicia automaticamente em 0 e começa um novo ciclo.</small>
    </div>

    <div className="v2-card">
      <div className="v2-label">Estratégia</div>
      <select className="v2-select" value={strategy} onChange={e => { setStrategy(e.target.value as Strategy); setTicks([]); setCycle(0); setSignal(null); }} disabled={running}>
        <option value="PAR_IMPAR">Par / Ímpar</option>
        <option value="ACIMA_ABAIXO">Acima 4 / Abaixo 5</option>
        <option value="RISE_FALL">Subir / Descer</option>
        <option value="PAR_ACIMA">Combinada: Par + Acima/Abaixo</option>
        <option value="PAR_RISE">Combinada: Par/Ímpar + Subir/Descer</option>
        <option value="ACIMA_RISE">Combinada: Acima/Abaixo + Subir/Descer</option>
        <option value="COMBINADA">Combinada: Paridade + Dígito + Direção</option>
      </select>
      <div className="v2-grid" style={{ marginTop:8 }}>
        <div><div className="v2-label">Stake USD</div><input className="v2-input" type="number" min="0.5" step="0.1" value={stake} onChange={e => setStake(Number(e.target.value))} disabled={running} /></div>
        <div><div className="v2-label">Duração</div><input className="v2-input" type="number" min="1" max="100" value={duration} onChange={e => setDuration(Number(e.target.value))} disabled={running} /></div>
      </div>
      <small style={{ color:'var(--t3)', display:'block', marginTop:7 }}>Para Even/Odd e Over/Under, a duração do contrato é limitada a 10 ticks; 25T/100T aqui são janelas de análise, não duração do contrato.</small>
    </div>

    <div className="v2-card">
      <div className="v2-label">Sinal atual</div>
      <div className="v2-signal">{signal ? `${signal.label} · ${signal.strength}%` : 'Aguardando fechar o próximo ciclo'}</div>
      <div style={{ color:'var(--t3)',fontSize:10,marginTop:4 }}>Par {Math.round(currentStats.even)}% · Ímpar {Math.round(currentStats.odd)}% · Acima {Math.round(currentStats.over)}% · Abaixo {Math.round(currentStats.under)}%</div>
    </div>
