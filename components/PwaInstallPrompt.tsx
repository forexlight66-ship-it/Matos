...
export default function PwaInstallPrompt() {
  ...
  if (!show) return null;
  const dismiss = () => { localStorage.setItem('matos-install-dismissed', '1'); setShow(false); };
  const install = async () => {
    if (!deferred) return;
  ...
  return <div style={{position:'fixed',inset:0,zIndex:9999,display:'flex',alignItems:'flex-end',justifyContent:'center',padding:'16px',background:'rgba(0,0,0,.45)'}}>
    <div style={{width:'100%',maxWidth:390,border:'1px solid #323738',borderRadius:18,padding:20,background:'#151717',color:'#fff',boxShadow:'0 20px 60px rgba(0,0,0,.5)'}}>
      <div style={{fontSize:20,fontWeight:700,marginBottom:6}}>📱 Instale o MatosFX</div>
      <div style={{fontSize:14,color:'#c2c2c2',lineHeight:1.5,marginBottom:16}}>Tenha acesso rápido ao MatosFX diretamente na tela inicial do seu celular.</div>
