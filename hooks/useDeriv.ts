    ...
    const start = async () => {
      try {
        ...
        if (!response.ok || !session?.wsUrl) {
          throw new Error(session?.error || `Unable to create Deriv WebSocket session (${response.status})`);
        }
        ...
        ws.subscribe('*', (data) => {
          ...
          if (data.error.code === 'AuthorizationRequired' || data.error.code === 'Unauthorized') {
