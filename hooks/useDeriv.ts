  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        ws.subscribe('profit_table', (data) => { if (data.profit_table) { setProfitTransactions(data.profit_table.transactions || []); setProfitCount(data.profit_table.count || 0); setLoadingProfit(false); } });
        ...
        ws.subscribe('proposal_open_contract', (data) => {
          ...
          if (c.is_sold || c.status === 'won' || c.status === 'lost') {
            const closed: ProfitTransaction = { contract_id:contractId, buy_price:buyPrice, sell_price:c.sell_price==null?null:Number(c.sell_price), payout, purchase_time:purchaseTime, sell_time:sellTime, contract_type:c.contract_type||'', longcode:c.longcode, profit_loss:profitLoss, exit_tick:c.exit_tick ?? null };
        ...
        if (!cancelled) { setIsConnected(false); setIsAuthorized(false); setError(err instanceof Error?err.message:'Unable to initialize Deriv connection'); }
      }
    };
    start();
