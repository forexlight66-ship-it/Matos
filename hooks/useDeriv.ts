...
function calculateProfitLoss(tx: Partial<ProfitTransaction>): number {
  ...
  // payout is the contract payout, not the user's profit. Never use
  // payout - buy_price as the primary closed-contract P/L calculation.
  return 0;
}
    ...
    for (const tx of incoming) {
      ...
      if (Number.isFinite(id) && id > 0) {
        const normalized: ProfitTransaction = {
          ...tx,
          contract_id: id,
          buy_price: Number(tx.buy_price ?? 0),
          sell_price: tx.sell_price == null ? null : Number(tx.sell_price),
          payout: Number(tx.payout ?? 0),
          profit_loss: calculateProfitLoss(tx),
        };
    ...
    const start = async () => {
      try {
        ...
        ws.subscribe('proposal_open_contract', data => {
          ...
          const sellPrice = c.sell_price == null ? null : Number(c.sell_price);
          const payout = Number(c.payout ?? 0);
          const profitLoss = Number.isFinite(Number(c.profit_loss))
            ? Number(c.profit_loss)
            : (sellPrice != null && Number.isFinite(buyPrice) ? sellPrice - buyPrice : 0);
