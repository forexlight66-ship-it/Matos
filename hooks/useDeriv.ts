...
export function useDeriv() {
  ...
  const mergeProfitTransactions=useCallback((incoming:ProfitTransaction[])=>{
    for(const tx of incoming){
      const id=Number(tx.contract_id); const purchaseTime=Number(tx.purchase_time ?? 0);
      ...
      const preservedExitTick = tx.exit_tick != null && String(tx.exit_tick) !== '' ? tx.exit_tick : previous?.exit_tick ?? null;
  ...
  useEffect(()=>{
    ...
    sessionStartedAtRef.current=Math.floor(Date.now()/1000); closedContractsRef.current.clear(); setProfitTransactions([]); setProfitCount(0);
      ...
      try{
        ...
        ws.subscribe('proposal_open_contract',data=>{
          ...
          if(c.is_sold||c.status==='won'||c.status==='lost'){
            ...
            setLoadingProfit(false); activeContractRef.current=null; latestProposalReqRef.current=null; setProposal(null); window.setTimeout(()=>{if(!cancelled)refreshProfitThrottled()},2000);
          }
        });
