...
function translateAutoBotStaticText(language:Language, root:Element){
  const tr=translations[language]||translations.en;
  const replacements:Record<string,string>={
    'Negocia mais rápido':tr.slogan,
    'Trade faster':tr.slogan,
    'Opera más rápido':tr.slogan,
    'Últimos Dígitos:':tr.lastDigits,
    'Last Digits:':tr.lastDigits,
    'Analisando':tr.analyzing,
    'Analyzing':tr.analyzing,
    'Analizando':tr.analyzing,
    'Aberto':tr.contractOpen,
    'Open':tr.contractOpen,
    'Abierto':tr.contractOpen,
    'Fechado':tr.contractClosedStage,
    'Closed':tr.contractClosedStage,
    'Cerrado':tr.contractClosedStage,
    'Moeda':tr.currency,
...
export function LanguageProvider({children}:{children:ReactNode}) {
  ...
  useEffect(()=>{
    const run=()=>{
      const root=document.querySelector('.av4');
      if(root) translateAutoBotStaticText(language,root);
    };
    run();
